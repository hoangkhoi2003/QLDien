        ...
        private void btnThemNhanVien_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = db.GetConnection())
            {
                try
                {
                    ...
                    // Kiểm tra xem khu vực đã có nhân viên quản lý chưa
                    string queryCheckQuanLy = @"
    SELECT TOP 1 MaNhanVien 
    FROM QuanLyKhachHang
    WHERE MaTinh = @PCTinh AND MaHuyen = @PCHuyen AND MaXa = @PCXa";

                    using (SqlCommand checkCmd = new SqlCommand(queryCheckQuanLy, conn))
                    {
                        checkCmd.Parameters.AddWithValue("@PCTinh", PCTinh);
                        checkCmd.Parameters.AddWithValue("@PCHuyen", PCHuyen);
                        checkCmd.Parameters.AddWithValue("@PCXa", PCXa);

                        object result = checkCmd.ExecuteScalar();

                        if (result != null)
                        {
                            MessageBox.Show("Khu vực này đã có nhân viên quản lý. Vui lòng chọn khu vực khác.");
                            return;
                        }
                    }
    ...
    VALUES (@TenNhanVien, @SoDienThoai, @Email, @MaTaiKhoan, @DiaChiCuThe, @Tinh, @Huyen, @Xa)";
            ...
            WHERE MaTaiKhoan = @MaTaiKhoan";
                    ...
                    using (SqlCommand getCmd = new SqlCommand(queryGetMaNhanVien, conn))
                    {
                        ...
                        {
                            MessageBox.Show("Không thể lấy mã nhân viên vừa thêm.");
                            return;
                        }
                    }

                    // Lấy danh sách khách hàng theo khu vực
