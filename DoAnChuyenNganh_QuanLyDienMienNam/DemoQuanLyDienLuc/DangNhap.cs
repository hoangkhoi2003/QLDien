    ...
    public partial class DangNhap : Form
    {
        ...
        private void btnLogin_Click_1(object sender, EventArgs e)
        {
            ...
            string sql = @"SELECT ChucVu 
                   ...
                   AND MatKhau = HASHBYTES('SHA2_256', CAST(@password AS NVARCHAR(50)))";
            ...
            using (SqlCommand cmd = new SqlCommand(sql, dbConnect.Conn))
            {
                ...
                if (reader.Read())
                {
                    ...
                    if (role == "Khách Hàng")
                    {
                        ...
                        string maKhachHang = LayMaKhachHang(username);

                        if (!string.IsNullOrEmpty(tenKhachHang) && !string.IsNullOrEmpty(maKhachHang))
                        {
                            string maHeThong = LayMaHeThong(maKhachHang);

                            if (!string.IsNullOrEmpty(maHeThong))
                            {
                                GUI_khachhang guiKhachHangForm = new GUI_khachhang(tenKhachHang, maKhachHang, maHeThong, role);
                                guiKhachHangForm.FormClosed += (s, args) => this.Show();
                                this.Hide();
                                guiKhachHangForm.ShowDialog();
                            }
                            else
                            {
                                MessageBox.Show("Không tìm thấy mã hệ thống cho khách hàng.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            TrangChu trangChuForm = new TrangChu(null, tenNhanVien, maNhanVien, role);
        ...
        private string LayMaKhachHang(string username)
        {
            DBConnect db = new DBConnect();
            ...
            {
                conn.Open();
                string query = @"SELECT kh.MaKhachHang 
                         FROM KhachHang kh 
                         INNER JOIN TaiKhoan tk ON kh.MaTaiKhoan = tk.MaTaiKhoan 
                         WHERE tk.TenDangNhap = @TenDangNhap";
                using (SqlCommand cmd = new SqlCommand(query, conn))
        ...
        private string LayMaHeThong(string maKhachHang)
        {
            string sql = "SELECT MaHeThong FROM HeThongDien WHERE MaKhachHang = @maKhachHang";
            using (SqlCommand cmd = new SqlCommand(sql, dbConnect.Conn))
            {
